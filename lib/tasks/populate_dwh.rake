namespace :db do
    ...
    task :fact_quote do
        ActiveRecord::Base.establish_connection(:adapter  => "mysql2",:host => ENV["HOST_MYSQL"],:port => ENV["PORT"], :username => ENV["MYSQL_USERNAME"],:password => ENV["MYSQL_PASSWORD"],:database => ENV["MYSQL_DATABASE"])
        row = ActiveRecord::Base.connection.execute(ENV["FACT_QUOTE_QUERY"])
        ActiveRecord::Base.establish_connection(:adapter  => "postgresql",:host => ENV["HOST_POSTGRESQL"],:port => ENV["PORT"],:username => ENV["POSTGRESQL_USERNAME"],:password => ENV["POSTGRESQL_PASSWORD"],:database => ENV["POSTGRESQL_DATABASE"])
        row.each do |val|
